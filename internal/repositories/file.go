...
func (repo *Repository) GetFileById(id, userId int) (file models.File, err error) {
	row := repo.Conn.QueryRow(context.Background(), `SELECT id, file_name, directory_id, user_id, created_at FROM files WHERE id = $1 AND user_id = $2`, id, userId)

	err = row.Scan(&file.Id, &file.FileName, &file.DirectoryId, &file.UserId, &file.CreatedAt)
	if err != nil {
